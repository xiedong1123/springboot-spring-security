...
(function() {
	lyGrid = (function(params,callback) {
		...
		//整理数据
		var prepare = function(confs,params){
		...
		var init = function(conf,callback) {
			...
			//fixhead();
			if(callback)
				//回调函数 column 字段名,当前界面的数据 currentData
				callback(column,currentData);
		};
		
		//构建html
		var createHtml = function(conf,returnData){
		...
		var cHeadTable = function(divid) {
			...
			th.setAttribute("style", "display: " + cn + ";");
			tr.appendChild(th);
			var cbk = "";
			if (!conf.checkbox) {
				cbk = "none";
			...
			cth.setAttribute("display: " + cbk + ";");
			var chkbox = document.createElement("INPUT");
			chkbox.type = "checkbox";
			chkbox.setAttribute("pagId", conf.pagId);
			chkbox.setAttribute("lay-skin", "primary");
			/*var chkDiv = document.createElement("div");
			chkDiv.setAttribute("class", "layui-unselect layui-form-checkbox");
			chkDiv.setAttribute("lay-skin", "primary");
			var chki = document.createElement("i");
			chki.setAttribute("class", "layui-icon");
			chki.innerHTML = "&#xe626;"*/
			//chkDiv.appendChild(chki);
			chkbox.onclick = checkboxbind.bind();
			cth.appendChild(chkbox); // 第一列添加复选框
			//cth.appendChild(chkDiv); 
			tr.appendChild(cth);
		...
		var cBodyTh = function(divid) {
			...
			if(!conf.isFixed){//不固定表头
				...
				var th = document.createElement('th');
				th.setAttribute("style", "text-align:center;width: 25px;vertical-align: middle;display: " + cn + ";");
				tr.appendChild(th);
				var cbk = "";
				if (!conf.checkbox) {
					cbk = "none";
							...
							if (l_tree.tree) {
								...
								if(CommnUtil.in_array(lt,clm)){
									...
									if(l_tree.hide||l_tree.hide!=undefined)
										img.src=rootPath+"/static/my/image/tree/nolines_plus.gif";
									else
										img.src=rootPath+"/static/my/image/tree/nolines_minus.gif";
		...
		var conf =  prepare(confs,params);
		var column = conf.l_column;
		var l_tree = conf.treeGrid;
		var returnData = jsonRequest(conf);		
		init(conf,returnData);
